...
document.addEventListener('DOMContentLoaded', () => {
          ...
          <div class="wuolah-note-item">
            <div class="note-header">
              <div class="note-title-wrap">
                <div class="note-verified-tag" title="Documento verificado y disponible sin error 404">
                  ...
                  <span>PDF Verificado (Sin 404)</span>
                </div>
                ...
                <div class="note-badges-row">
                  ...
                  <span class="note-tag note-tag-size">${note.fileSize || '3 KB'}</span>
                </div>
              </div>
            ...
            <div class="note-footer">
              ...
              <div class="note-action-btns">
                <button type="button" class="btn-pdf-view" onclick="window.openPdfViewer('${sub.id}', '${note.id}')" title="Ver contenido del PDF en visor interactivo">
                ...
                <a href="${note.pdfPath}" download="${sub.acronym}_${note.id}.pdf" class="btn-pdf-download" title="Descargar este archivo PDF directamente (Sin error 404)">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <article class="subject-card" style="--card-accent-color: ${sub.color};">
          ...
          <div class="card-actions-footer">
            <button class="btn-action-primary" onclick="window.openSubjectModal('${sub.id}')">
              ...
              Ver Temario y 3 PDFs
            </button>
            <a href="${sub.bestWuolahNotes[0].pdfPath}" download="${sub.acronym}_top_apuntes.pdf" class="btn-action-icon" title="Descargar PDF principal de esta asignatura">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
            ...
            <a href="https://wuolah.com" target="_blank" rel="noopener noreferrer" class="btn-action-icon" title="Comunidad oficial Wuolah">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon>
              </svg>
            </a>
            <button class="btn-action-icon" onclick="window.copySubjectShareLink('${sub.id}')" title="Copiar nombre de asignatura">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  function handleLaunchFinderSearch() {
    ...
    // Buscar si disponemos del PDF directo para este tipo de documento
    ...
    if (matchedNote) {
      showToast(`✓ Abriendo PDF directo: "${matchedNote.title}"`);
      window.openPdfViewer(subject.id, matchedNote.id);
    } else {
      window.openSubjectModal(subject.id);
    }
  }

  // Helper global para abrir la comunidad de Wuolah de forma segura sin 404
  window.openWuolahCommunity = function(searchPhrase) {
    if (searchPhrase && navigator.clipboard) {
      navigator.clipboard.writeText(decodeURIComponent(searchPhrase)).then(() => {
        showToast(`📋 Nombre copiado: "${decodeURIComponent(searchPhrase)}". Pégalo en el buscador de Wuolah.`);
      }).catch(() => {});
    } else {
      showToast("Abriendo portal oficial de Wuolah...");
    }
    window.open("https://wuolah.com", "_blank", "noopener,noreferrer");
  };
  ...
  window.openSubjectModal = function(subjectId) {
    ...
    // Lista de PDFs verificados para el modal de asignatura
    const wuolahModalHtml = subject.bestWuolahNotes.map(n => {
      return `
        <div class="wuolah-note-item" style="margin-bottom: 10px;">
          <div class="note-header">
            <div class="note-title-wrap">
              <div class="note-verified-tag" title="Documento verificado y disponible sin error 404">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="note-badges-row">
                ...
                <span class="note-tag">★ ${n.rating} (${n.reviewsCount} valoraciones)</span>
                <span class="note-tag">${n.pages} págs</span>
                <span class="note-tag note-tag-size">${n.fileSize || '3 KB'}</span>
              </div>
            </div>
            <div class="note-action-btns">
              <button type="button" class="btn-pdf-view" onclick="window.openPdfViewer('${subject.id}', '${n.id}')" title="Ver documento en visor interactivo">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path>
                  <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path>
                </svg>
              ...
              <a href="${n.pdfPath}" download="${subject.acronym}_${n.id}.pdf" class="btn-pdf-download" title="Descargar este archivo PDF directamente">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                  <polyline points="7 10 12 15 17 10"></polyline>
                  <line x1="12" y1="15" x2="12" y2="3"></line>
    ...
    modalBodyContent.innerHTML = `
      ...
      <div style="display: flex; gap: 12px; margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--border-subtle); justify-content: flex-end; flex-wrap: wrap;">
        ...
        <a href="https://wuolah.com" target="_blank" rel="noopener noreferrer" class="header-btn header-btn-wuolah">
          Comunidad Wuolah
        </a>
      </div>
    `;
  ...
  window.copySubjectShareLink = function(subjectId) {
    ...
    const textToCopy = `${subject.name} UMA Informática Wuolah`;
    if (navigator.clipboard) {
      navigator.clipboard.writeText(textToCopy).then(() => {
        showToast(`📋 Copiado al portapapeles: "${textToCopy}"`);
      }).catch(() => {
        prompt('Copia el siguiente texto para buscar en Wuolah:', textToCopy);
      });
    } else {
      prompt('Copia el siguiente texto para buscar en Wuolah:', textToCopy);
    }
  ...
  window.openPdfViewer = function(subjectId, noteId) {
    ...
    if (pdfModalSubtitle) {
      pdfModalSubtitle.textContent = `[${subject.code}] ${subject.name} (${subject.acronym}) • Autor: @${note.uploader} • ${note.pages} páginas • ${note.fileSize || '3.0 KB'}`;
    }

    if (btnPdfModalDownload) {
      btnPdfModalDownload.href = note.pdfPath;
      btnPdfModalDownload.setAttribute('download', `${subject.acronym}_${note.id}.pdf`);
    }

    if (btnPdfModalOpenTab) {
      btnPdfModalOpenTab.href = note.pdfPath;
    }
      ...
      pdfModalBody.innerHTML = `
        <div class="pdf-viewer-content">
          ...
          <div class="pdf-status-alert">
            ...
            <div>
              <div class="pdf-status-title">Documento PDF Verificado y Disponible</div>
              <div class="pdf-status-desc">
                Este archivo PDF está integrado directamente en la plataforma. Puedes consultarlo aquí mismo o descargarlo de forma 100% garantizada sin enlaces rotos ni error 404 en Wuolah.
              </div>
            </div>
          </div>
          ...
          <div class="pdf-meta-pills">
            <span class="pdf-pill"><strong>Asignatura:</strong> ${subject.name}</span>
            ...
            <span class="pdf-pill"><strong>Formato:</strong> PDF 1.4 Estándar</span>
          </div>

          <!-- Document Preview Canvas / Simulated Reader Frame -->
          <div class="pdf-document-paper">
            <div class="paper-header">
              <div class="paper-brand">
                <span class="paper-tag">UNIVERSIDAD DE MÁLAGA • ETSI INFORMÁTICA</span>
                <h2 class="paper-title">${note.title}</h2>
                <div class="paper-sub">Plan Docente Oficial 2023 • Grado en Ingeniería del Software (1º Curso)</div>
              </div>
            </div>
            ...
            <div class="paper-section paper-tips-box">
              <h4 class="paper-section-title" style="color: #f59e0b;">
                ...
                Estrategia y Consejos para el Examen
              </h4>
              <p>${subject.examTips}</p>
            </div>

            <div class="paper-download-cta">
              <div>
                <h4 style="margin: 0 0 4px 0; color: #fff; font-size: 1.05rem;">¿Deseas descargar el archivo PDF a tu dispositivo?</h4>
                <p style="margin: 0; font-size: 0.84rem; color: var(--text-muted);">Puedes guardarlo localmente para consultarlo sin conexión o imprimirlo para tus sesiones de estudio.</p>
              </div>
              <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                <a href="${note.pdfPath}" download="${subject.acronym}_${note.id}.pdf" class="btn-pdf-action btn-pdf-primary">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                  Descargar Archivo PDF
                </a>
                <a href="${note.pdfPath}" target="_blank" rel="noopener noreferrer" class="btn-pdf-action btn-pdf-secondary">
